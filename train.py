"""
Train TransformerClassifier (sensor or video) with LOSO cross-validation.
...
def main() -> None:
    ...
    parser.add_argument("--modality", choices=["sensor", "video"], required=True)
    ...
    for fold_idx, test_subject in enumerate(subjects):
        if args.no_validation:
            ...
            # Circular validation: previous subject in the subjects list
        ...
        # Convert best Lightning checkpoint to .pt state_dict
        # Use trainer's checkpoint callback to get the ACTUAL best checkpoint
...
# Backwards compatibility aliases for part scripts
