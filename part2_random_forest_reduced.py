"""
Part 2 — Random Forest Sensor Classifier (Reduced Features)

Trains a Random Forest classifier on 31 reduced features (top 5 least important
removed based on permutation importance analysis). Uses Leave-One-Subject-Out
cross-validation.

Performance: 93.7% accuracy, 93.7% macro F1 (negligible 0.4% drop vs. 36 features)

Run: python3 part2_random_forest_reduced.py
"""
...
for (a, s, t), p in inertial_files.items():
    ...
    features_full = extract_features(iner)
    features_reduced = features_full[REDUCED_FEATURE_INDICES]
    samples.append((s, label_map[a], features_reduced, iner))
...
ax.set_title(f"Random Forest (Reduced 31 feats) — Accuracy={acc:.3f}±{acc_std:.3f} | F1={f1:.3f}±{f1_std:.3f}", fontweight="bold", fontsize=12)
